...
MemorialPage, the two pages built as a distinct "editorial" visual subsystem (still
the only consumers of `react-wrap-balancer`). Framer Motion, once confined to those
two pages, is now used app-wide through `src/lib/motion.tsx` — see Animation tokens.
...
**The motion system lives in `src/lib/motion.tsx`.** It is the only file allowed to
import from `framer-motion` (it mounts `LazyMotion` in strict mode, so a stray
`motion.div` anywhere else throws). Everything else imports from it:

```tsx
import { m, AnimatePresence, EASE_LUX, fadeInUp, staggerContainer } from '../lib/motion';
```

It exports:

- `m` / `AnimatePresence` — the sanctioned animated elements
- `EASE_LUX` — the house curve, `cubic-bezier(0.16, 1, 0.3, 1)` (mirrors `--ease-lux`)
- `DUR` — durations in seconds (`fast` 0.15, `base` 0.25, `slow` 0.4, `reveal` 1)
- Variants: `fadeUp`/`stagger` (editorial, Login/Memorial), `fadeInUp`/`staggerContainer`/
  `listItem` (in-app content), `scalePop` (modals/popovers), `pageVariants` (routes)
- `useCountUp` / the `AnimatedNumber` component in `ui.tsx` — animated stat values
- `MotionProvider` — mounted once in `App.tsx`; its `MotionConfig reducedMotion="user"`
  disables transform animation for users with reduced-motion enabled

**When to animate:** motion must communicate state (loading, success, hierarchy,
causality), run ≤ ~400ms, and collapse to nothing under `prefers-reduced-motion`
(CSS animation is covered by the global reduced-motion block at the bottom of
`src/styles/index.css`). No perpetual/looping attention-seeking animation. The one
sanctioned celebration is the `.gold-shimmer` one-shot sweep (grant marked received).

CSS-side durations (used by Tailwind `transition-*` utilities):

| Variable | Value | Tailwind class | Use for |
|---|---|---|---|
| `--transition-fast` | 150ms | `duration-fast` | Hover states, color changes |
| `--transition-base` | 200ms | `duration-DEFAULT` | Small reveals |
| `--transition-slow` | 300ms | `duration-slow` | Sheets, larger moves |

Predefined CSS animations (`animate-fade-in`, `animate-slide-up`, `animate-scale-in`)
are generated by Tailwind from keyframes in `index.css`; prefer the Framer variants
above for anything that needs an exit animation or stagger.
