  ...
  return (
    <div className={`border rounded-lg p-4 ${styles[variant]}`}>
      <div className="flex items-start justify-between gap-4">
        ...
        {onDismiss && (
          <button onClick={onDismiss} className="text-current/70 hover:text-current" aria-label="Dismiss alert">
            ✕
          </button>
        )}
      </div>
...
export function Badge({ children, variant = 'primary', size = 'md', dot = false }: BadgeProps) {
  ...
  const dotColors = {
    ...
    danger: 'bg-danger', info: 'bg-info', secondary: 'bg-foreground-muted', outline: 'bg-foreground-muted',
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ...
        className={`
          ...
          ${sizes[size]} w-full max-h-[90vh] flex flex-col outline-none animate-scale-in
        `}
      >
