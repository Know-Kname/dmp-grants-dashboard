...
export const getErrorMessage = (error: unknown, fallback = 'Something went wrong') => {
  if (isApiError(error)) return friendlySupabaseMessage(error.message) || fallback;
  if (error instanceof Error && error.message) return friendlySupabaseMessage(error.message);
  return fallback;
};
...
export const getErrorRequestId = (error: unknown) => {
  if (isApiError(error) && error.requestId) return error.requestId;
  return null;
};
